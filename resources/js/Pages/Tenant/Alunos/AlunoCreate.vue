<script setup>
import { Head, Link, useForm } from '@inertiajs/vue3';
import TenantLayout from '@/Layouts/TenantLayout.vue';
import FormInput from '@/Components/Ui/Form/FormInput.vue';
import { ref } from 'vue';

const props = defineProps({
    planos: Array,
});

const form = useForm({
    nome: '',
    email: '',
    cpf: '',
    data_nascimento: '',
    status: 'ativo',
    plano_id: '',
});

const submit = () => {
    form.post('/alunos', {
        onSuccess: () => form.reset(),
    });
};

const statusOptions = ref([
    { value: 'ativo', label: 'Ativo' },
    { value: 'inativo', label: 'Inativo' },
    { value: 'suspenso', label: 'Suspenso' },
]);
</script>

<template>
    <Head title="Novo Aluno" />

    <TenantLayout title="Cadastrar Novo Aluno">
        <div class="max-w-4xl mx-auto bg-white shadow rounded-lg p-8">
            <h2 class="text-xl font-bold text-gray-800 mb-6 border-b pb-3">Dados Pessoais do Membro</h2>

            <form @submit.prevent="submit">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">

                    <FormInput
                        id="nome"
                        label="Nome Completo"
                        placeholder="Nome e Sobrenome"
                        v-model="form.nome"
                        :error="form.errors.nome"
                        type="text"
                        required
                    />

                    <FormInput
                        id="email"
                        label="Email"
                        placeholder="exemplo@email.com"
                        v-model="form.email"
                        :error="form.errors.email"
                        type="email"
                        required
                    />

                    <FormInput
                        id="cpf"
                        label="CPF"
                        placeholder="000.000.000-00"
                        v-model="form.cpf"
                        :error="form.errors.cpf"
                        type="text"
                    />

                    <FormInput
                        id="data_nascimento"
                        label="Data de Nascimento"
                        v-model="form.data_nascimento"
                        :error="form.errors.data_nascimento"
                        type="date"
                    />

                    <!-- Select de Status -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Status *</label>
                        <select
                            v-model="form.status"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500"
                            :class="{ 'border-red-500': form.errors.status }"
                        >
                            <option
                                v-for="option in statusOptions"
                                :key="option.value"
                                :value="option.value"
                            >
                                {{ option.label }}
                            </option>
                        </select>
                        <div v-if="form.errors.status" class="text-xs text-red-500">{{ form.errors.status }}</div>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Plano</label>
                        <select
                            v-model="form.plano_id"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500"
                            :class="{ 'border-red-500': form.errors.plano_id }"
                        >
                            <option value="">Selecione um plano...</option>
                            <option
                                v-for="p in planos"
                                :key="p.id"
                                :value="p.id"
                            >
                                {{ p.nome }}
                            </option>
                        </select>
                        <div v-if="form.errors.plano_id" class="text-xs text-red-500">{{ form.errors.plano_id }}</div>
                    </div>

                </div>

                <div class="mt-8 pt-5 border-t flex justify-end gap-3">
                    <Link href="/alunos" class="px-6 py-3 text-gray-700 rounded-lg hover:bg-gray-100">Cancelar</Link>

                    <button type="submit" :disabled="form.processing"
                        class="px-6 py-3 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700 disabled:opacity-50">
                        {{ form.processing ? 'Salvando...' : 'Cadastrar Aluno' }}
                    </button>
                </div>
            </form>
        </div>
    </TenantLayout>
</template>